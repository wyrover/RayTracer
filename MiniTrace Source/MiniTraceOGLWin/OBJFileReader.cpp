...
static int secondPassOBJRead(LPCWSTR filename, int nVerts, int nNormals, int nTexcoords, Triangle* mesh)
{
	...
	do
	{
		...
		if (*v == 'v')
		{
			...
			{
				float x, y, z;
				//sscanf_s(v + 1, "%f %f %f", &(vertex_buffer[vertex_read][0]), &(vertex_buffer[vertex_read][1]), &(vertex_buffer[vertex_read][2]));
				sscanf_s(v + 1, "%f %f %f", &x, &y, &z);
				vertex_buffer[vertex_read] = Vector3(x, y, z);
				vertex_read += 1;
			}
		}
		else if (*v == 'f')
		{
			...
			if (sscanf_s(v, "%*s %d/%d/%d %d/%d/%d %d/%d/%d", &index1, &tidx1, &nidx1, &index2, &tidx2, &nidx2, &index3, &tidx3, &nidx3) == 9)
			{
				...
				mesh[triangle_read].SetTriangle(vertex_buffer[index1 - 1], vertex_buffer[index2 - 1], vertex_buffer[index3 - 1]);
			}
			else
			{
